...
public class RecommendationManager {
    ...
    private Recommendations loadRecs() {
        try {
            ...
            recFile.close();
            return TvApp.getApplication().getSerializer().DeserializeFromString(json, Recommendations.class);
        } catch (IOException e) {
            // none saved
            ...
            public void onResponse(ItemsResult response) {
                if (response.getTotalRecordCount() > 0) {
                    for (BaseItemDto episode : response.getItems()) {
                        if (episode.getLocationType() != LocationType.Virtual) {
                            addRecommendation(episode, RecommendationType.Tv);
                        }
                    }
                }
            }
    ...
    private void addRecommendation(BaseItemDto item, RecommendationType type) {
        //No need if we are already there
        if (mRecommendations.get(type, item.getId()) != null) {
            return;
        }
